# Week 1 Role Catalog

Each person should choose one main role and, if needed, one support role.

| Role | What this role does | Starter task |
|---|---|---|
| Main Leader | Runs the team, keeps the work moving, and removes blockers | Open the kickoff meeting and assign the first tasks |
| Frontend Developer | Builds screens, layout, and user interactions | Create the first app screen |
| Backend Developer | Builds APIs, data handling, auth, and server logic | Stub the first endpoint or data model |
| UI/UX Designer | Makes wireframes, user flows, and interface improvements | Draw the first wireframe |
| DevOps Engineer | Sets up repo structure, CI, deployment, and release checks | Add the repository workflow and release checklist |
| QA Tester | Writes tests, finds bugs, and checks that fixes work | Create a smoke test checklist |
| Documentation Writer | Writes README files, setup steps, and release notes | Draft the project intro and setup guide |
| Community Manager / Presenter | Explains the project, prepares slides, and helps with voting | Write a 60-second project pitch |
| Maintainer / Release Manager | Reviews changes, tags releases, and keeps the handoff clean | Make the release checklist |

## Role Rules

- One person can hold one main role and one support role if the team is small
- Roles can change at weekly review time if the team agrees
- The title should match the real work, not just a cool name
